...
def display_frame(frame, bbox_roi, frame_count, flag_tracker_active):
...
def main():
    ...
    for frame_number in range(NB_FRAMES):
        ...
        if flag_tracker_active:  # The tracker is ON
            ...
            bbox_roi = [int(value) for value in bbox_roi]  # The update returns floating point values...
            if frame_number%CNN_CHECK_STRIDE == 0:
                ...
                prediction, crop = infer_bbox(loaded_model, current_frame, bbox_roi, METHOD)
                if prediction == 1:  # All good, keep going
                    flag_quit_program = display_frame(current_frame, bbox_roi, frame_number, flag_tracker_active)
                    #fps.update()
        ...
        for contour in contours:
            ...
            label = 1 if bbox.intersection_over_union(converted_bbox, converted_gt_bbox) >= params['iou'] else 0
...
if __name__ == '__main__':
    ...
    VIDEO_STREAM, NB_FRAMES, FRAME_WIDTH, FRAME_HEIGHT = init.import_stream(VIDEO_STREAM_PATH)
