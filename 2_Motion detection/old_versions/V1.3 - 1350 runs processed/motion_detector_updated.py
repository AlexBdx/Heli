...
while True:
	...
	if frame is not None:
		...
		break
vs = vs_cache
print("Loading: ", time.perf_counter()-t0)
nFrames = len(vs)
...
"""
# Create a log file
if args.get("restart", None) is None:
	with open("log2.csv", 'w') as f:
		w = csv.writer(f)
		w.writerow(["gaussWindow", "mgp", "minArea", "diffMethod", "winSize", "maxLevel"]+["realFps", "avNbBoxes", "avNbFilteredBoxes", "avNbHeliBox", "percentHeliTotalFiltered", "percentFrameWithHeli"])
	iterationStart = 0
else:
...
for gaussWindow in range(3, 12, 2):#tqdm.tqdm(range(3, 12, 2)): # 5
	for mgp in range (25, 151, 25):#range (25, 151, 25): # 6
		for minArea in [x**2 for x in range(1, 6)]:#[x**2 for x in range(1, 6)]: # 5
			for diffMethod in range(0, 2): # 1
				for winSize in range(13, 30, 8): # range(13, 30, 8) # 3
					for maxLevel in range(1, 8, 3): # range(1, 8, 3) # 3
...
# Shuffle the work so you don't get trapped in a boring location for ever
iterations = np.random.permutation(iterations)
print("[INFO] Starting {} iterations".format(len(iterations)))

for simulation in tqdm.tqdm(iterations):
	#-------------------------------------
	# RESET THE SIM DEPENDENT VARIABLES
	#-------------------------------------
	nbBoxes = []
	...
	#while True:
		...
		print("Access: ", time.perf_counter()-t0)
		assert 1==0
	...
	simOutput = simulation+[realFps, avNbBoxes, avNbFilteredBoxes, avNbHeliBox, percentHeliTotalFiltered, percentFrameWithHeli]
